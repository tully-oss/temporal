...
func (s *RawHistoryClientSuite) TestGetHistoryReverse() {
	...
	wfeResponse, err := env.SdkClient().DescribeWorkflowExecution(ctx, workflowRun.GetID(), workflowRun.GetRunID())
	s.NoError(err)
	...
	events := s.getHistoryReverse(env, env.Namespace().String(), wfeResponse.WorkflowExecutionInfo.Execution, 100)
	s.Len(events, len(eventDefaultOrder))
	s.Equal(eventDefaultOrder, events)

	events = s.getHistoryReverse(env, env.Namespace().String(), wfeResponse.WorkflowExecutionInfo.Execution, 3)
	s.Len(events, len(eventDefaultOrder))
	s.Equal(eventDefaultOrder, events)

	events = s.getHistoryReverse(env, env.Namespace().String(), wfeResponse.WorkflowExecutionInfo.Execution, 1)
	s.Len(events, len(eventDefaultOrder))
	s.Equal(eventDefaultOrder, events)
}
...
func (s *RawHistoryClientSuite) TestGetHistoryReverse_MultipleBranches() {
	...
	rweResponse, err := env.SdkClient().ResetWorkflowExecution(ctx, &workflowservice.ResetWorkflowExecutionRequest{
		Namespace:                 env.Namespace().String(),
	...
	resetWorkflowRun := env.SdkClient().GetWorkflow(ctx, wfId, resetRunId)
	err = resetWorkflowRun.Get(ctx, nil)
	s.NoError(err)

	resetWfeResponse, err := env.SdkClient().DescribeWorkflowExecution(ctx, resetWorkflowRun.GetID(), resetWorkflowRun.GetRunID())
	s.NoError(err)
	...
	events := s.getHistoryReverse(env, env.Namespace().String(), resetWfeResponse.WorkflowExecutionInfo.Execution, 100)
	s.Len(events, len(eventsDefaultOrder))
	s.Equal(eventsDefaultOrder, events)

	events = s.getHistoryReverse(env, env.Namespace().String(), resetWfeResponse.WorkflowExecutionInfo.Execution, 3)
	s.Len(events, len(eventsDefaultOrder))
	s.Equal(eventsDefaultOrder, events)

	events = s.getHistoryReverse(env, env.Namespace().String(), resetWfeResponse.WorkflowExecutionInfo.Execution, 1)
	s.Len(events, len(eventsDefaultOrder))
	s.Equal(eventsDefaultOrder, events)
}
...
func (s *RawHistoryClientSuite) getHistoryReverse(env *testcore.TestEnv, namespace string, execution *commonpb.WorkflowExecution, pageSize int32) []*historypb.HistoryEvent {
