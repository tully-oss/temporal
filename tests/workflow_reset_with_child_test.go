...
func (s *WorkflowResetWithChildSuite) newTestEnv(opts ...testcore.TestOption) *testcore.TestEnv {
	env := testcore.NewEnv(s.T(), opts...)
	env.SdkWorker().RegisterWorkflow(workflowWithChildren)
	env.SdkWorker().RegisterWorkflow(workflowWithWaitingChild)
	env.SdkWorker().RegisterWorkflow(child)
	env.SdkWorker().RegisterWorkflow(waitingChild)
...
func (s *WorkflowResetWithChildSuite) TestResetWithChild() {
	...
	options := s.startWorkflowOptions(env, wfID)
	run, err := env.SdkClient().ExecuteWorkflow(s.Context(), options, workflowWithChildren, false, false)
	s.NoError(err)
	...
	childIDs := s.getChildWFIDsFromHistory(env, wfID, run.GetRunID())
	s.Len(childIDs, 3)
	...
	err = env.SdkClient().GetWorkflow(s.Context(), wfID, resp.GetRunId()).Get(s.Context(), &resultAfterReset1)
	s.NoError(err)
	s.Equal(originalResult, resultAfterReset1)

	childIDsAfterReset1 := s.getChildWFIDsFromHistory(env, wfID, resp.GetRunId())
	...
	resetRequest.WorkflowTaskFinishEventId = s.getWorkflowTaskFinishEventIDAfterChild(env, wfID, resp.GetRunId(), childIDsAfterReset1[0].WorkflowId)
	resp, err = env.SdkClient().ResetWorkflowExecution(s.Context(), resetRequest)
	s.NoError(err)

	// Wait for the new run to complete.
	var resultAfterReset2 string
	err = env.SdkClient().GetWorkflow(s.Context(), wfID, resp.GetRunId()).Get(s.Context(), &resultAfterReset2)
	s.NoError(err)
	s.Equal(originalResult, resultAfterReset2)

	childIDsAfterReset2 := s.getChildWFIDsFromHistory(env, wfID, resp.GetRunId())
	...
	resetRequest.WorkflowExecution.RunId = resp.GetRunId()
	resetRequest.WorkflowTaskFinishEventId = s.getWorkflowTaskFinishEventIDAfterChild(env, wfID, resp.GetRunId(), childIDsAfterReset2[1].WorkflowId)
	resp, err = env.SdkClient().ResetWorkflowExecution(s.Context(), resetRequest)
	s.NoError(err)

	// Wait for the new run to complete.
	var resultAfterReset3 string
	err = env.SdkClient().GetWorkflow(s.Context(), wfID, resp.GetRunId()).Get(s.Context(), &resultAfterReset3)
	s.NoError(err)
	s.Equal(originalResult, resultAfterReset3)

	childIDsAfterReset3 := s.getChildWFIDsFromHistory(env, wfID, resp.GetRunId())
	...
	resetRequest.WorkflowExecution.RunId = resp.GetRunId()
	resetRequest.WorkflowTaskFinishEventId = s.getWorkflowTaskFinishEventIDAfterChild(env, wfID, resp.GetRunId(), childIDsAfterReset3[2].WorkflowId)
	resp, err = env.SdkClient().ResetWorkflowExecution(s.Context(), resetRequest)
	s.NoError(err)
	childIDsFinal := s.getChildWFIDsFromHistory(env, wfID, resp.GetRunId())
...
func (s *WorkflowResetWithChildSuite) TestResetWithChild_WithChildID() {
	...
	run, err := env.SdkClient().ExecuteWorkflow(s.Context(), options, workflowWithChildren, true, false)
	s.NoError(err)
	var originalResult string
	err = run.Get(s.Context(), &originalResult)
	s.NoError(err)
	...
	initialChildExecutions := s.getChildWFIDsFromHistory(env, wfID, run.GetRunID())
	...
	childExecutionsAfterReset1 := s.getChildWFIDsFromHistory(env, wfID, resp.GetRunId())
	...
	resetRequest.RequestId = "reset-request-2"
	resetRequest.WorkflowExecution.RunId = resp.GetRunId()
	resetRequest.WorkflowTaskFinishEventId = s.getWorkflowTaskFinishEventIDAfterChild(env, wfID, resp.GetRunId(), childExecutionsAfterReset1[0].WorkflowId)
	resp, err = env.SdkClient().ResetWorkflowExecution(s.Context(), resetRequest)
	s.NoError(err)
	...
	childExecutionsAfterReset2 := s.getChildWFIDsFromHistory(env, wfID, resp.GetRunId())
	...
	resetRequest.RequestId = "reset-request-3"
	resetRequest.WorkflowExecution.RunId = resp.GetRunId()
	resetRequest.WorkflowTaskFinishEventId = s.getWorkflowTaskFinishEventIDAfterChild(env, wfID, resp.GetRunId(), childExecutionsAfterReset2[1].WorkflowId)
	resp, err = env.SdkClient().ResetWorkflowExecution(s.Context(), resetRequest)
	s.NoError(err)
	// Wait for the new run to complete.
	var resultAfterReset3 string
	...
	childExecutionsAfterReset3 := s.getChildWFIDsFromHistory(env, wfID, resp.GetRunId())
	...
	resetRequest.RequestId = "reset-request-4"
	resetRequest.WorkflowExecution.RunId = resp.GetRunId()
	resetRequest.WorkflowTaskFinishEventId = s.getWorkflowTaskFinishEventIDAfterChild(env, wfID, resp.GetRunId(), childExecutionsAfterReset3[2].WorkflowId)
	resp, err = env.SdkClient().ResetWorkflowExecution(s.Context(), resetRequest)
	s.NoError(err)

	// Wait for the new run to complete.
	var resultAfterReset4 string
	err = env.SdkClient().GetWorkflow(s.Context(), wfID, resp.GetRunId()).Get(s.Context(), &resultAfterReset4)
	s.NoError(err)
...
func (s *WorkflowResetWithChildSuite) TestResetWithChild_WithChildID_WithRejectDuplicate() {
	...
	run, err := env.SdkClient().ExecuteWorkflow(s.Context(), options, workflowWithChildren, true, true)
	s.NoError(err)
	var originalResult string
	err = run.Get(s.Context(), &originalResult)
	s.NoError(err)

	s.verifyReusePolicyIsSetForAllChild(env, wfID, run.GetRunID(), enumspb.WORKFLOW_ID_REUSE_POLICY_REJECT_DUPLICATE)
	...
	resetRequest.RequestId = "reset-request-4"
	resetRequest.WorkflowExecution.RunId = resp.GetRunId()
	resetRequest.WorkflowTaskFinishEventId = s.getWorkflowTaskFinishEventIDAfterChild(env, wfID, resp.GetRunId(), initialChildExecutions[2].WorkflowId)
	resp, err = env.SdkClient().ResetWorkflowExecution(s.Context(), resetRequest)
	s.NoError(err)
...
func (s *WorkflowResetWithChildSuite) TestResetWithChild_RunningChild_RandomWID() {
	...
	options := s.startWorkflowOptions(env, wfID)
	firstRun, err := env.SdkClient().ExecuteWorkflow(s.Context(), options, workflowWithWaitingChild, false, false)
	s.NoError(err)

	// save child init initialChildExecutions for later comparison.
	var initialChildExecutions []*commonpb.WorkflowExecution
	s.Eventually(func() bool {
		initialChildExecutions = s.getChildWFIDsFromHistory(env, wfID, firstRun.GetRunID())
...
func (s *WorkflowResetWithChildSuite) TestResetWithChild_RunningChild_SetWID() {
	...
	options := s.startWorkflowOptions(env, wfID)
	firstRun, err := env.SdkClient().ExecuteWorkflow(s.Context(), options, workflowWithWaitingChild, true, false)
	s.NoError(err)
...
func (s *WorkflowResetWithChildSuite) TestResetWithChild_RunningChild_SetWID_WithRejectDuplicate() {
	...
	options := s.startWorkflowOptions(env, wfID)
	firstRun, err := env.SdkClient().ExecuteWorkflow(s.Context(), options, workflowWithWaitingChild, true, true)
	s.NoError(err)
	...
	s.verifyReusePolicyIsSetForAllChild(env, wfID, firstRun.GetRunID(), enumspb.WORKFLOW_ID_REUSE_POLICY_REJECT_DUPLICATE)
...
func (s *WorkflowResetWithChildSuite) TestResetWithChild_AfterStartingChild() {
	...
	s.Eventually(func() bool {
		...
		resetRequest.WorkflowTaskFinishEventId = s.getWorkflowTaskFinishEventIDAfterChildInit(env, wfID, firstRun.GetRunID(), initialChildExecutions[0].WorkflowId)
...
func (s *WorkflowResetWithChildSuite) TestResetWithChild_AfterChildCompletes() {
	...
	s.Eventually(func() bool {
		return s.getWorkflowTaskFinishEventIDAfterChild(env, wfID, firstRun.GetRunID(), initialChildExecutions[0].WorkflowId) != 0
	}, 5*time.Second, 100*time.Millisecond)
...
func (s *WorkflowResetWithChildSuite) TestResetWithChild_AfterChildTerminated() {
	...
	s.Eventually(func() bool {
		wftAfterChildTerminated = s.getWorkflowTaskFinishEventIDAfterChild(env, wfID, firstRun.GetRunID(), initialChildExecutions[0].WorkflowId)
...
func (s *WorkflowResetWithChildSuite) startWorkflowOptions(env *testcore.TestEnv, wfID string) sdkclient.StartWorkflowOptions {
...
func workflowWithChildren(ctx workflow.Context, fixedWID bool, rejectDuplicatePolicy bool) (string, error) {
...
func workflowWithWaitingChild(ctx workflow.Context, fixedWID bool, rejectDuplicatePolicy bool) (string, error) {
	wfID := workflow.GetInfo(ctx).WorkflowExecution.ID
	var result string
	opt := workflow.ChildWorkflowOptions{}
	if rejectDuplicatePolicy {
	...
	childCtx := workflow.WithChildOptions(ctx, opt)
	err := workflow.ExecuteChildWorkflow(childCtx, waitingChild, "hello child").Get(ctx, &result)
	if err != nil {
		return "", err
	}
	return result, err
...
func waitingChild(ctx workflow.Context, arg string) (string, error) {
	ctx = workflow.WithActivityOptions(ctx, defaultActivityOptions())
